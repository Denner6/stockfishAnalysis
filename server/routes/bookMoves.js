...
var bookMovesJson = require("./bookMoves.json");
var fens = [
...
function removeMoveCountFromFen(fen) {
    var fenSplit = fen.split(" ");
    fenSplit = fenSplit.slice(0, -2);
    return fenSplit.join(" ");
}
var BookMoves = /** @class */ (function () {
    function BookMoves() {
    }
    BookMoves.getBookMove = function (fen) {
        fen = removeMoveCountFromFen(fen);
        for (var _i = 0, bookMovesJson_1 = bookMovesJson; _i < bookMovesJson_1.length; _i++) {
            var bookMoveObject = bookMovesJson_1[_i];
            var fenWithoutCount = removeMoveCountFromFen(bookMoveObject.fen);
    ...
    BookMoves.getBookMoves = function (fens) {
        var bookMoves = [];
        for (var _i = 0, fens_1 = fens; _i < fens_1.length; _i++) {
            var fen = fens_1[_i];
            var bookMoveObject = this.getBookMove(fen);
            if (bookMoveObject)
                bookMoves.push(bookMoveObject);
        }
        return bookMoves;
    };
    return BookMoves;
}());
exports.default = BookMoves;
