import { Engine as NodeUciEngine} from "node-uci";
import { Chess } from "chess.js";

 
interface BestsMoves {
    bestMove: string,
    fen: string
}

class Engine {
    private static engineOptions = {
        MultiPv: "1",
        Threads: "2",
        Hash: "15"
    }

    private static async setupEngine() {
        const engine = new NodeUciEngine("./stockfish/stockfish-exec");

        engine.init()

        for (const [option, value] of Object.entries(this.engineOptions)) {
            await engine.setoption(option, value)
        }
    ...
    private static convertBestMoveToFenString(move: string, fen: string): string {
        const chess = new Chess(fen);
        chess.move(move);
        return chess.fen();
    }

    static async getBestMoves(fenArray: string[]): Promise<BestsMoves[]> {
